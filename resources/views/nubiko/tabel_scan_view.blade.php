    ...
    <style>
        ...
        *::after {
        ...
        .app-sidebar.closed-sidebar {
            width: 80px;
            /* Adjust the width when the sidebar is closed */
            min-width: 80px;
            overflow: hidden;
        }
        ...
        .search h1,
.search input {
    margin: 10px; /* Hapus margin bawaan */
    margin-left: 30px; /* Hapus margin bawaan */

}

.search h1 {
    flex: 0.5; /* Bagi ruang secara merata antara elemen h1 */
    white-space: nowrap; /* Hindari pemisahan teks ke baris baru */
}

.search input {
    flex: 2; /* Bagi ruang secara merata antara elemen input */
    margin-right:15%;
}
        ...
        .judul-atas {
            display: grid;
            grid-template-columns: 0fr 2fr 5fr;
            padding-bottom: 60px;
...
<body>
    ...
    <div class="app-container app-theme-white body-tabs-shadow fixed-sidebar fixed-header">
        ...
        <div class="app-main">
            <div class="container">
                <div class="tabel-scan-view">
                    <h1>TABEL SCAN VIEW</h1>
                </div>
                <div class="search">
                    <h1>Id Scan View</h1>
                ...
                <div class="btn-scan">
                    <form action="{{ route('scan.simpan') }}" method="post">
                        @csrf
                        <input type="hidden" name="filteredData" value="{{ json_encode($filteredData) }}">
                        <input type="hidden" name="expedisi" value="{{ request('expedisi') }}">
                        ...
                        <table class="table table-hover">
                            <thead class="bg-secondary">
                                <tr>
                                    ...
                                    <th>Marketplace</th>
                                    <th>No Resi</th>
                                    <th id="aksiColumn" style="display: none;">Hapus</th>
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                <tr>
                                    @if($loop->first)
                                    <!-- Display Expedisi and Marketplace only in the first row -->
                                    <td rowspan="{{ $filteredData->count() }}">{{ $data->nama_expedisi
                                        }}</td>
                                    <td rowspan="{{ $filteredData->count() }}">{{
                                        $data->nama_marketplace }}</td>
                                    @endif
                                    <td data-resi>{{ $data->no_resi }}</td>

                                    <!-- Tambahkan kolom aksi hanya jika is_duplicate -->
                                    <td>
                        ...
                        <button type="submit" class="btn btn-primary" @if($filteredData->isEmpty() ||
                            $filteredData->contains('is_duplicate', true)) style="display:none"
                            @endif>Simpan</button>

                        <script>
                            document.addEventListener('DOMContentLoaded', function () {
                                var noResiList = document.querySelectorAll('td[data-resi]');
                                var noResiValues = Array.from(noResiList).map(td => td.textContent.trim());

                                var duplicates = findDuplicates(noResiValues);

                                if (duplicates.length > 0) {
                                    var audio = new Audio("{{ asset('audio/alarm.mp3') }}");
                                    audio.play();
                                    alert('ALARM: TERDAPAT NO RESI YANG SAMA!\nNO RESI: ' + duplicates.join(', '));

                                    // Menampilkan kolom "Aksi"
                                    document.getElementById('aksiColumn').style.display = 'table-cell';
                                }
                            });
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ...
                document.querySelector('.close-sidebar-btn').addEventListener('click', function () {
                    ...
                    var isSidebarClosed = appSidebar.classList.contains('closed-sidebar');

                    // Adjust app-main__inner and app-page-title based on sidebar state
                    adjustAppLayout(isSidebarClosed);
                });

                // ... (code for other event listeners)

                // Function to adjust app-main__inner and app-page-title based on sidebar state
                function adjustAppLayout(isSidebarClosed) {
                    var appMainInner = document.querySelector('.app-main__inner');
                    var appPageTitle = document.querySelector('.app-page-title');

                    if (isSidebarClosed) {
                        // If sidebar is closed, hide app-page-title and make app-main__inner full width
                        appPageTitle.style.display = 'none';
                        appMainInner.style.width = '100%';
                    } else {
                        // If sidebar is open, show app-page-title and adjust width as needed
                        appPageTitle.style.display = 'block';
                        // You can adjust this value based on your design
                        appMainInner.style.width = 'calc(100% - [sidebar-width]px)';
                    }
                }
            });
        </script>
