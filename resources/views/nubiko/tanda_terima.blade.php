...
<head>
    ...
    <style>
        ...
        *::after {
        ...
        .container-menu-tracking h1,
        .container-menu-tracking input {
            margin: 10px;
            /* Hapus margin bawaan */
            margin-left: 30px;
            /* Hapus margin bawaan */
        ...
        .container-menu-tracking h1 {
            flex: 0.5;
            /* Bagi ruang secara merata antara elemen h1 */
            white-space: nowrap;
            /* Hindari pemisahan teks ke baris baru */
        }

        .container-menu-tracking input {
            flex: 2;
            /* Bagi ruang secara merata antara elemen input */
            margin-right: 15%;
        }
                ...
                <div class="app-main__inner">
                    ...
                    <div class="container">
                        <div class="container-menu-tracking">
                            <form method="get" action="{{ route('tanda_terima') }}" id="filterForm">
                                <select name="nama_expedisi" onchange="this.form.submit()">
                                    <option value="" {{ request('nama_expedisi')=='' ? 'selected' : '' }}>-- Select
                                        Expedisi --</option>
                                    @foreach ($expedisiOptions as $option)
                                    <option value="{{ $option }}" {{ request('nama_expedisi')==$option ? 'selected' : ''
                                        }}>{{ $option }}</option>
                                    @endforeach
                                </select>
                            </form>

                            <div class="Id-Bukti-Tanda-Terima">
                                <form method="get" action="" class="float-right">
                                    <h1>Id Bukti Tanda Terima</h1>
                                    <input type="text" name="id_tanda_terima" id="id_tanda_terima" class="form-control"
                                        value="{{ request('id_tanda_terima') }}">
                                </form>
                            </div>
                        </div>

                        <div class="btn-scan">
    <div id="divToShowHide">
    <form action="{{ route('print.pdf') }}" method="post">
    @csrf
    @if(request('nama_expedisi') || request('id_tanda_terima'))
        <table class="table table-bordered table-striped">
            <thead>
                <tr>
                    ...
                    <th>Tanggal</th>
                    <th>Durasi</th>
                    <th>{{ request('nama_expedisi')}}</th>
                    <th>Select</th>
                </tr>
            </thead>
            <tbody>
                @foreach ($tandaTerima as $index => $data)
                    <tr>
                        <td>{{ $index + 1 }}</td>
                        ...
                        <td>
                        <input type="checkbox" name="selectedData[{{ $data->tanggal }}][]" value="{{ $data->nama_produk }}">
<input type="hidden" name="tanggal[{{ $data->tanggal }}][]" value="{{ $data->tanggal }}">
<input type="hidden" name="durasi[{{ $data->tanggal }}][]" value="{{ $data->durasi }}">
<input type="hidden" name="total_resi[{{ $data->tanggal }}][]" value="{{ $data->total_resi }}">
        ...
        <input type="hidden" name="nama_expedisi" value="{{ request('nama_expedisi') }}">
